...
Community 0: codex/pr8-importacao-csv-seguranca, main, 0687af4 feat(privacy): implementar endpoints de exportação J, 077b08e feat(privacy): adicionar migration 010 para pseudoni, 0e1c4ad test: consolidar QA, acessibilidade e release do Fon, 2dd873d feat: consolidar financeiro único para dois CNPJs (#, 4de92ce Graphify, 557b254 fix(deploy): corrigir render.yaml para sintaxe válid, 5ccde91 feat: implementar atalhos rápidos de comunicação via, 639a914 feat(privacy): adicionar botão de portabilidade e an, 68c033b chore: inicializar main para entregas independentes, 75e42fd fix(ui): aplicar flex min-width 0 e empilhamento em  [lang=pt]
Community 1: normalizePhone(, validPatientPhone(, 0d2db0f feat: centralizar cadastro e histórico de pacientes , attachments.ts, ALLOWED_MIME_TYPES, AllowedMimeType, calculateFileHash(, sanitizeFilename(, validFileSize(, validMimeType(, csv-import.ts, calculateCsvHash( [lang=nl]
...
Community 4: hashPassword(, 3a29c99 chore(deploy): preparar infraestrutura no Render com, 3f9e6cc feat: estabelecer núcleo seguro e executável da clín, create-admin.ts, email, migrate.ts, migrate(, pool.ts, pool, seed.ts, seedDemo(, security.ts [lang=pt]
Community 5: a740db5 feat: registrar vendas com parcelas e pós-venda auto, finance.ts, ENTRY_TYPES, FINANCE_CATEGORIES, validFinancialEntry(, sales.ts, DELIVERY_STATUSES, PAYMENT_METHODS, SaleInstallment, splitMonthly(, validateInstallments(, validCents( [lang=pt]
